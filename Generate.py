...
def Generate(signal,x,Amount,normalized=True,fourier=False,jump=False,N=40,type='Trig',Noise=False,noiseParameter=0.1,method='ifft'):
    signalOutput=[]
    length=Amount+2
        ...
        if fourier:
            for i in range(Amount):
                if(jump):
                    ...
                    signalOutput.append(FourierSeries(cn[0:-1],x,method=method))
                normalization=1#np.max(abs(F))
                #BoxSignal.append(F/normalization)
                #BoxSignalFourier.append(fx)
