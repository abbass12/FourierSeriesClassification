...
def extract_jump_features(signal: np.ndarray, x: np.ndarray,
                          n_modes: int = 50,
                          sigma_type: str = 'trig',
                          max_jumps: int = 4) -> np.ndarray:
    """
    Extract jump discontinuity features from a signal.

    Returns a fixed-length feature vector containing jump locations
    and magnitudes, zero-padded to max_jumps.
    ...
    Returns:
        Feature vector of length 2 * max_jumps (locations + values)
    """
    ...
    # Create fixed-length feature vector (zero-padded)
    jump_features = np.zeros(2 * max_jumps)

    n_detected = min(len(locations), max_jumps)
    if n_detected > 0:
        # Sort by magnitude (strongest jumps first)
        sort_idx = np.argsort(-np.abs(values))[:n_detected]
        jump_features[:n_detected] = locations[sort_idx]
        jump_features[max_jumps:max_jumps + n_detected] = values[sort_idx]
...
def signals_to_fourier_with_jumps(X: np.ndarray, x: np.ndarray,
    ...
    Returns:
        Feature array of shape (n_samples, 2 * n_modes + 2 * max_jumps)
    """
    n_samples = X.shape[0]
